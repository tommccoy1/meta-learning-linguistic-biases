...
Each of these should generate a meta-training set in `data/PREFIX.train`, a meta-dev set in `data/PREFIX.dev`, and a meta-test set in `data/PREFIX.test`, where `PREFIX` is given by the `--output_prefix` argument. This works by expanding out the keys in the `data/` directory into full datasets.

yonc.train should start with:
```
...
In addition, the exact `yonc.dev` and `yonc.test` files have been provided in `data/`, as `yonc_original.dev` and `yonc_original.test`. You can confirm by doing a diff.
